...
use Dktaylor\BundleGeneratorBundle\Symfony\Maker\Adapter\UseStatementsAdapter;
use Dktaylor\BundleGeneratorBundle\Symfony\Maker\SafeNameResolverInterface;
...
final readonly class ScaffoldContext
{
    public function __construct(
        ...
        public UseStatementsInterface $useStatements,
    ) {}
    ...
    public static function fromInput(
        ...
        SafeNameResolverInterface $safeNameResolver,
    ): self {
        ...
        $bundleDir = self::resolveBundleDirectory(
            $generator->getRootDirectory(),
            $bundleFullName, $safeNameResolver
        );
     ...
     */
    private static function resolveBundleDirectory(
        string $rootDirectory,
        string $bundleFullName,
        SafeNameResolverInterface $safeNameResolver,
    ): string {
        // Strip any path separators from the bundle name before use in a path
        $safeName = $safeNameResolver->resolve($bundleFullName);
    ...
    private static function buildUseStatements(): UseStatementsInterface
    {
        return new UseStatementsAdapter([
